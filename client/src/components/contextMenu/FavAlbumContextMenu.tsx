...
import { fetchFavoriteAlbums, unlikeAlbum } from "../../store/reducers/action-creators/album.ts";

interface FavAlbumContextMenuProps {
    contextMenu: { mouseX: number; mouseY: number } | null;
    onClose: () => void;
    album: AlbumSimpleDto;
}

export const FavAlbumContextMenu: React.FC<FavAlbumContextMenuProps> = ({
    contextMenu,
    onClose,
    album
}) => {
    ...
    const handleRemoveFromFavorites = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (userId) {
            dispatch(unlikeAlbum({ albumId: album.id, userId: userId })).then(()=>{
