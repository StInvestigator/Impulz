...
import { fetchFavoritePlaylists, unlikePlaylist } from "../../store/reducers/action-creators/playlist.ts";

interface FavPlaylistContextMenuProps {
    contextMenu: { mouseX: number; mouseY: number } | null;
    onClose: () => void;
    playlist: PlaylistDto;
}

export const FavPlaylistContextMenu: React.FC<FavPlaylistContextMenuProps> = ({
    contextMenu,
    onClose,
    playlist
}) => {
    ...
    const handleRemoveFromFavorites = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (userId) {
            dispatch(unlikePlaylist({ playlistId: playlist.id, userId: userId })).then(() => {
